...
# while True: # realvid
	...
	if run_threads==1 and frame is not None:
		...
		current_freq=w1.get()
		if remotetrack == -1 or w1.get() == 0 or time.time()-time_start>160:#or output_q_cnt== onecatvidlen*3:
			threadLock.acquire()
			every_n_frame['n']=-1
			threadLock.release()
		...
		if True:#not output_q.empty():
			...
			if output_q_cnt==0: 
				 checked.set(str(sys.argv[8]))	
			if "RT" in sys.argv[7]:
				if time.time()-time_start >=40 and sent_cnt==0:#output_q_cnt == onecatvidlen  :
					sock_client.send(bytes('L','UTF-8'))
					w1.set(FSIZE[0][1])
					sent_cnt+=1

				if time.time()-time_start >=80 and sent_cnt==1:#output_q_cnt == 2*onecatvidlen:
					sock_client.send(bytes('M','UTF-8'))
					sent_cnt+=1
					w1.set(FSIZE[1][1])

				if time.time()-time_start >=120 and sent_cnt==2:#output_q_cnt == 2*onecatvidlen:
