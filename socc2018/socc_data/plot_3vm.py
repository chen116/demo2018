...
for f in files:
    ...
    for i in range(len(x)):
        ...
        if i==1:
            ncpu= (100-np.asarray(cpus[i])-np.asarray(cpus[0])).tolist()
            ax2.plot(cpus_xs[i],ncpu,"-",lw=((i+1)%2)+1,label= sched[2],color='teal' )
    ...
    ax1.plot([0 ,120],[10 ,10],'r',label= 'Deadline')
    ...
    ax1.set_ylabel('Window Heart Rate(FPS)',fontsize=14)
    ax2.set_ylabel('Assigned CPU Utilization(%)',fontsize=14)
    ax1.set_ylim(7, 18 )  
    ax2.set_ylim( -5, 105 )  
    ax1.set_xlim(-5,125)
    ax2.set_xlim(-5,125)
    ...
    if show_frames:
        for i in range(len(frame_xs)):
            ...
            for j in range(len(frame_xs[i])):
                ...
                ymin,ymax=ax1.get_ylim()
                if inn%2==0:
                    ax1.text(frame_xs[i][j],22.,"Person Detected",rotation=45,fontdict=font[i])
                else:
                    ax1.text(frame_xs[i][j],23.3,"Person Leaves Frame",rotation=45,fontdict=font[i])
    ...
    fig.savefig('../writing/images/'+ f.split('.txt')[0]+'.pdf', bbox_inches='tight')
