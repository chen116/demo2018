...
if catlen>0: 
	...
	if "RT" in sys.argv[7]:
		...
		vidarray = np.concatenate((car,car,man,man),axis=0)
	else:
		...
		vidarray = np.concatenate((car,man,man,car,car),axis=0)
	print(len(vidarray))
	...
	if run_threads==1 and frame is not None:
		...
		if True:#not output_q.empty():
			...
			if object_detected == 'person' and w1.get()!=FSIZE[2][1]:
				w1.set(FSIZE[2][1])
			if object_detected == 'car' and w1.get()!=FSIZE[1][1] :
				w1.set(FSIZE[1][1])
