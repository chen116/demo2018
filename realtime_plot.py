...
ax_improvement_percentage_txt = ax_improvement_percentage.text(0.02,0.01,'%.2f%%'%(0),fontdict=font_per[1])
ax_improvement_percentage.axis('off')

ax_improvement_percentage_vs_static = plt.axes([0.7, 0.91, 0.2, 0.12])
ax_improvement_percentage_vs_static.text(0.06,0.42,'meowRT-Xen outperforms Credit by:',fontdict=font_per[0])
    ...
    if show_frames:
        for i in range(len(frame_xs)):
            for j in range(len(frame_xs[i])):
                ...
                ax2.text(frame_xs[i][j],25,"frame: "+str(frames[i][j]),rotation=45,fontdict=font[i],horizontalalignment='right',verticalalignment='top')
